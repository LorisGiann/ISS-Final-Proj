...
internal class TestSprint1_mover {
    ...
    fun goTo(pos : ws.Position) {
        try {
            ...
            val truckRequestStr = "msg(move, request,python,mover,move($pos),1)"
    ...
    fun goBackHome() = goTo(ws.Position.HOME)
    ...
    fun test_from_H_to_H() {
        assertTimeoutPreemptively<Unit>(Duration.ofSeconds(10)){
            ...
            try {
                ...
                val truckRequestStr = "msg(move, request,python,mover,move(HOME),1)"
                val answer = connTcp.request(truckRequestStr)
                ...
                Assertions.assertTrue(to!!.checkNextContent("mover(handle,HOME,HOME)") >= 0)
                Assertions.assertTrue(to!!.checkNextContent("mover(wait,HOME,HOME)") >= 0)
                ...
                Assertions.fail("test_from_H_to_H ERROR:" + e.message)
            }
        }
    }
    ...
    fun test_from_H_to_I() {
        assertTimeoutPreemptively<Unit>(Duration.ofSeconds(20)){
            ...
            try {
                ...
                val truckRequestStr = "msg(move, request,python,mover,move(INDOOR),1)"
                val answer = connTcp.request(truckRequestStr)
                ...
                // --------------------------------------------------------------------- CHECKING HISTORY ---------------------------------------------------------------------
                Assertions.assertTrue(to!!.checkNextContent("mover(handle,HOME,INDOOR)") >= 0)
                Assertions.assertTrue(to!!.checkNextContent("mover(req_forward_aclk,HOME,INDOOR)") >= 0)
                ...
                Assertions.fail("test_from_H_to_I ERROR:" + e.message)
            }
        }
    }
    ...
    fun test_from_PB_to_I() {
        assertTimeoutPreemptively<Unit>(Duration.ofSeconds(30)){
            ...
            try {
                ...
                Assertions.assertTrue(to!!.checkNextContent("mover(req_forward_clk,PLASTICBOX,INDOOR)") >= 0)
                Assertions.assertTrue(to!!.checkNextContent("mover(wait,INDOOR,INDOOR)") >= 0)
                ...
                Assertions.fail("test_from_PB_to_I ERROR:" + e.message)
            }
        }
    }
    ...
    fun test_from_I_to_GB() {
        assertTimeoutPreemptively<Unit>(Duration.ofSeconds(30)){
            ...
            try {
                ...
                val truckRequestStr = "msg(move, request,python,mover,move(GLASSBOX),1)"
                val answer = connTcp.request(truckRequestStr)
                ColorsOut.outappl("test_accepted answer=$answer", ColorsOut.GREEN)
                Assertions.assertTrue(answer.contains("moveanswer(OK)"))
                ...
                Assertions.assertTrue(to!!.checkNextContent("mover(handle,PLASTICBOX,GLASSBOX)") >= 0)
                Assertions.assertTrue(to!!.checkNextContent("mover(wait,GLASSBOX,GLASSBOX)") >= 0)

            } catch (e: Exception) {
                Assertions.fail("test_from_I_to_GB ERROR:" + e.message)
            }
        }
    }
    ...
    fun test_new_pos_no_chenage_route_from_GB_to_H_then_I() { //initially a GB->H move is commanded, but halfway through the nel position I is commanded: route should not change (no u turns)
        assertTimeoutPreemptively<Unit>(Duration.ofSeconds(30)){
            ...
            try {
                ...
                //FIRST REQUEST
                var truckRequestStr = "msg(move, request,python,mover,move(HOME),1)"
                connTcp.forward(truckRequestStr)
                to!!.waitUntilState("mover","mover(req_forward_aclk,GLASSBOX,HOME)") //wait for the robot to proceed from PLASTICBOX to HOME
                CommUtils.delay(250)  //wait to be roughly in the middle of the wall

                //SECOND REQUEST
                truckRequestStr = "msg(move, request,python,mover,move(INDOOR),1)"
                val answer = connTcp.request(truckRequestStr)
                ...
                Assertions.assertTrue(to!!.checkNextContent("mover(set_new_dest_aclk,GLASSBOX,INDOOR)") >= 0)
                ...
                Assertions.fail("test_new_pos_no_chenage_route_from_GB_to_H_then_I ERROR:" + e.message)
            }
        }
    }
    ...
    fun test_new_pos_chenage_route_from_PB_to_H_then_I() { //initially a PB->H move is commanded, but halfway through the nel position I is commanded: route should change (u turn)
        assertTimeoutPreemptively<Unit>(Duration.ofSeconds(30)){
            ...
            try {
                ...
                val currIndex = to!!.nextCheckIndex
                Assertions.assertTrue(to!!.checkNextContent("mover(set_new_dest_aclk,PLASTICBOX,INDOOR)") >= 0)
                Assertions.assertTrue(to!!.checkNextContent("mover(handle,PLASTICBOX,INDOOR)") >= 0)
                Assertions.assertTrue(to!!.checkNextContent("mover(handle,INDOOR,INDOOR)") >= 0)
                to!!.nextCheckIndex = currIndex
