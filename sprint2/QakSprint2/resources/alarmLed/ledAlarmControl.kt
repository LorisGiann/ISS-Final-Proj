			...
			try {
				...
				when (RESOURCE) {
					...
					"basicrobotwrapper" -> {
						...
						ISHALTED = checkIsHaltedBasicRobotWrapper(STATE)
						println("$tt $name | basicrobotwrapper in $STATE, halted = $ISHALTED" )
					}
					"pickupdropouthandler" -> {
						val msgterm = (Term.createTerm(VALUE) as Struct)
						val STATE = msgterm.getArg(0).toString()
						ISHALTED = checkIsHaltedPickupDropout(STATE)
						println("$tt $name | pickupdropouthandler in $STATE, halted = $ISHALTED" )
					}
				}
