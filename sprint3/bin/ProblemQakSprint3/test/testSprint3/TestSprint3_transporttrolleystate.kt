...
internal class TestSprint2_integration_transporttrolleystate {
    ...
    fun test_accepted() {
        assertTimeoutPreemptively<Unit>(Duration.ofSeconds(90)) {
            try {
                ...
                //alarm is received but state is still MOVING
                Assertions.assertTrue(to!!.checkNextContent("transporttrolleystate(HALT)") >= 0) //then after turn is's in halt state
                Assertions.assertTrue(to!!.checkNextContents(arrayOf( //alarm ceased, now it's time for the pickup
                    "alarmreceivertest(wait,alarmceased)",
